#导入数据集
import pandas as pd
iris_data = pd.read_csv('171DT20220927.csv')
#指定列名
iris_data.columns = ['Y', 'X1', 'X2', 'X3', 'X4']
iris_data.head()
...
sb.pairplot(iris_data.dropna(), hue='Y')#不能有缺失值
plt.figure(figsize=(10, 10))
for column_index, column in enumerate(iris_data.columns):
    if column == 'Y':
        continue
    plt.subplot(2, 2, column_index + 1)
    sb.violinplot(x='Y', y=column, data=iris_data)#小提琴图
...
all_classes = iris_data['Y'].values
...
from sklearn.model_selection import cross_val_score  # 交叉验证交叉验证用于评估模型的预测性能，尤其是训练好的模型在新数据上的表现，可以在一定程度上减小过拟合。还可以从有限的数据中获取尽可能多的有效信息。
import numpy as np
