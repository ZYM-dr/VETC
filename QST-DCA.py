...
if __name__ == '__main__':
    ...
    #     'D:/DOCZ 2021-2025/Gd-EOB-DTPA ralated/Gd-EOB-DTPA DATA/RadiomicDATA20220404/yushiyan1/0.84-0.70gpc3-20220816-mse-cv5-1se.xlsx',
    #     sheet_name='CR-test')
    ...
    # # 在测试集上预测
    # # 模型4
    estimator_lr = LogisticRegression(C=1.0,
    ...
    # estimator_dt.fit(X_train2, y_train2)
    # y_score_test_dt2 = estimator_dt.predict_proba(X_test2)[:, 1]  # 预测概率
    ...
    # estimator_svm.fit(X_train2, y_train2)
    # y_score_test_svm2 = estimator_svm._predict_proba_lr(X_test2)[:, 1].tolist() 模型1
    ...
    # estimator_rf.fit(X_train2, y_train2)
    # y_score_test_rf2 = estimator_rf.predict_proba(X_test2)[:, 1]
    # print('y_score_test_rf:', y_score_test_rf.tolist())

    estimator_xgboost = XGBClassifier(n_estimators=9, max_depth=4, gamma=0.5)
    estimator_xgboost.fit(X_train1, y_train1)  # 训练集上建模
    y_score_test_xgboost1 = estimator_xgboost.predict_proba(X_train1)[:, 1]  # 预测概率
    # estimator_xgboost.fit(X_train2, y_train2)  # 训练集上建模
    # y_score_test_xgboost2 = estimator_xgboost.predict_proba(X_test2)[:, 1]  # 预测概率
    # print('y_score_test_xgboost:', y_score_test_xgboost1.tolist())
    # print('y_score_test_xgboost:', y_score_test_xgboost2.tolist())

    # 模型6
    estimator_knn = KNeighborsClassifier(n_neighbors=9, weights='uniform', metric_params=None, n_jobs=None)
    estimator_knn.fit(X_train1, y_train1)
    y_score_test_knn1 = estimator_knn.predict_proba(X_train1)[:, 1]
    # estimator_knn.fit(X_train2, y_train2)
    # y_score_test_knn2 = estimator_knn.predict_proba(X_test2)[:, 1]
    ...
    plot_DCA(y_label=y_train1,
             ...
             save_path=os.path.join(save_dir, 'dca-train.pdf'))
exit()
# print('验证集DCA')
# # 模型4
...
# # y_score_test_lr2 = estimator_lr.predict_proba(X_train2)[:, 1]
# # 模型2
# estimator_dt = DecisionTreeClassifier(max_depth=5)
# estimator_dt.fit(X_test1, y_test1)
# y_score_test_dt1 = estimator_dt.predict_proba(X_test1)[:, 1]  # 预测概率
...
# y_score_test_svm1 = estimator_svm._predict_proba_lr(X_test1)[:, 1].tolist()
...
# y_score_test_rf1 = estimator_rf.predict_proba(X_test1)[:, 1]  # 预测概率
...
# y_score_test_xgboost1 = estimator_xgboost.predict_proba(X_test1)[:, 1]  # 预测概率
...
#              # ['RF2', y_score_test_rf2],
#          ],
#          save_path=os.path.join(save_dir, 'dca2.pdf'))
